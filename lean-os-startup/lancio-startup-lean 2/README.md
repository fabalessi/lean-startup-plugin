# Lancio Startup Lean — Lean OS per founder
...
- `references/` — `metodo-lean-startup.md` (definizioni, tipi di MVP, 10 pivot, 3 motori), `fasi-coaching.md` (domande ed errori per fase), `cruscotti-operativi.md` (direttiva dashboard), `ritmo-operativo.md` (sprint/briefing/EOD/task), `resume-e-deliverable.md` (resume + deliverables).
- `assets/` — 4 template documentali (registro ipotesi, lean canvas, scheda esperimento, pivot-or-persevere) + `dashboard-template.html` (l'app-shell del backend).
