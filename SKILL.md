...
Deliverable: `assets/registro-ipotesi.md` (lista ipotesi ordinate per rischio) + `assets/lean-canvas.md`. Se l'utente lavora in un vault Obsidian (o lo chiede), proponi la variante visuale: installa il template `assets/obsidian-lean-canvas/` nel vault seguendo `references/lean-canvas-obsidian.md`.
...
## Lean Canvas visuale in Obsidian

La skill include in `assets/obsidian-lean-canvas/` un template pronto del Lean Canvas per Obsidian (9 blocchi `.md` + file `.canvas`, basato su YJPL/lean-canvas-for-obsidian, MIT). Quando l'utente vuole il canvas dentro il suo vault Obsidian — o dice "installa il lean canvas in Obsidian" — **leggi `references/lean-canvas-obsidian.md`** ed esegui la procedura: individua il vault (cartella `.obsidian`), copia i file in una sottocartella `Lean Canvas/`, adatta i percorsi dei nodi nel `.canvas` e verifica l'apertura. Vale la regola della fonte unica: o il canvas Obsidian o `assets/lean-canvas.md`, non entrambi compilati in parallelo.
